...
extension MultiTool {
    ...
    public final class Builder {
        ...
        /// Queues `capability` in place of the first recorded capability that
        /// `isReplaced` selects. When no recorded capability matches, it
        /// queues `capability` at the end, as `withCapability(_:)` does.
        ///
        /// The replacement keeps the position of the earlier registration,
        /// and gives no error. A short form that holds at most one
        /// capability of its kind calls this, so the last call wins. The
        /// storage stays `private`: this method is the only way that code
        /// out of this file changes a registration that is recorded.
        ///
        /// Internal, and not `private`, because the capability short forms in
        /// `MultiToolBuilder+Capabilities.swift` call it from another file.
        ///
        /// - Parameters:
        ///   - capability: the capability to register.
        ///   - isReplaced: tells whether a recorded capability is the one
        ///     that `capability` replaces.
        @discardableResult
        func withCapability(
            _ capability: any Capability,
            replacing isReplaced: (any Capability) -> Bool
        ) -> Self {
