...
- [ ] Each suite named above declares a limit of about three times its measured
      time, and no suite declares more than 15 minutes.
- [ ] `swift test --package-path IntegrationTests --no-parallel` passes, with
      the whole-package time recorded in a comment on this card.
- [ ] The largest ceiling any one suite declares is 15 minutes, thus a wedged
...
- [ ] The gated suite is the test. Run it one time and record what it took.
- [ ] No new test is needed: this changes a ceiling and no behaviour.
