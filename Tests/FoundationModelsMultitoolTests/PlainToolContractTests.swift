...
@Suite struct PlainToolContractTests {
    ...
    private static func seed(_ directory: URL) throws -> String {
    ...
    private static func verb<Verb: Tool>(_ type: Verb.Type, root: URL) throws -> Verb {
    ...
    private static func expectOutsideRoot(_ correction: String?) throws {
    ...
    /// The bytes on disk at a path, as UTF-8 text.
    ///
    /// - Parameter path: the absolute path to read.
    /// - Returns: the file's text, or `nil` when it does not exist.
    ...
    @Test func readAnswersContentWithNoContextBound() async throws {
        ...
        let path = try Self.seed(root)

        let result = try await Self.verb(Read.self, root: root)
    ...
    @Test func readOutsideTheRootIsCorrective() async throws {
        let root = Self.makeRoot()
        let outsidePath = try Self.seed(Self.makeRoot())

        let result = try await Self.verb(Read.self, root: root)
            .call(arguments: ReadArguments(path: outsidePath, offset: nil, limit: nil, format: nil))

        try Self.expectOutsideRoot(result.correction)
    }
    ...
    @Test func writeOutsideTheRootIsCorrective() async throws {
        ...
        try Self.expectOutsideRoot(result.correction)
        #expect(Self.text(at: outsidePath) == nil)
    }
    ...
    @Test func editRewritesTheFileWithNoContextBound() async throws {
        ...
        let result = try await Self.verb(Edit.self, root: root)
            .call(
        ...
        #expect(Self.text(at: path)?.contains(Self.editedWord) == true)
    }
    ...
    @Test func editOutsideTheRootIsCorrective() async throws {
        ...
        try Self.expectOutsideRoot(result.correction)
        #expect(Self.text(at: outsidePath) == Self.seededText)
    }
    ...
    @Test func patchAddsTheFileWithNoContextBound() async throws {
        ...
        let result = try await Self.verb(Patch.self, root: root)
            .call(arguments: PatchArguments(patch: Self.addFileEnvelope(path: path)))

        #expect(result.correction == nil)
        #expect(result.status == EditOutcomeProjection.appliedStatus)
        #expect(Self.text(at: path)?.contains(Self.addedText) == true)
    }
    ...
    @Test func globFindsTheFileWithNoContextBound() async throws {
        let root = Self.makeRoot()
        _ = try Self.seed(root)

        let result = try await Self.verb(Glob.self, root: root)
            .call(
    ...
    @Test func grepFindsTheLineWithNoContextBound() async throws {
        ...
        let result = try await Self.verb(Grep.self, root: root)
