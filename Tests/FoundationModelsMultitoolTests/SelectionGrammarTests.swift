...
struct SelectionGrammarTests {
    ...
    @Test("idEnumGrammar(ids:) fed a real registry's entry paths — SearchToolsTool.init(registry:librarian:limit:)'s own derivation — constrains the enum to exactly those paths, qualified paths included")
